...
reset_kafka_cluster() {
    ...
    docker compose -f docker/services/kafka.yml stop kafka >/dev/null 2>&1 || true
    docker compose -f docker/services/kafka.yml rm -f -s kafka >/dev/null 2>&1 || true
    ...
    docker compose -f docker/services/kafka.yml up -d kafka
...
docker compose -f docker/services/debezium-connect.yml up -d
...
if [ -f "./scripts/infra/debezium/setup-debezium-connectors.sh" ]; then
    ./scripts/infra/debezium/setup-debezium-connectors.sh
    print_success "Debezium connectors configured!"
else
    print_warning "Debezium setup script not found at ./scripts/infra/debezium/setup-debezium-connectors.sh, skipping..."
fi
...
if docker ps --format '{{.Names}}' 2>/dev/null | grep -q 'trackops-mirror-maker-2'; then
    ...
    if [ -f "./scripts/infra/mm2/setup-mm2-connectors.sh" ]; then
...
if [ "$USE_DOCKER" = "true" ]; then
    ...
    start_service_docker "Order Service" "docker/services/trackops-server.yml"
    start_service_docker "Inventory Service" "docker/services/inventory-service.yml"
    start_service_docker "Event Relay Service" "docker/services/event-relay-service.yml"
...
echo "  ./scripts/infra/stop-all-microservices.sh"
echo ""
echo "📋 To view logs:"
...
echo "  - Use ./scripts/infra/stop-all-microservices.sh to stop everything"
echo ""
echo "🐳 To start services via Docker instead of Gradle:"
echo "  USE_DOCKER=true ./scripts/infra/start-all-microservices.sh"
