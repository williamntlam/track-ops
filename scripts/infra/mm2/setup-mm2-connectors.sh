...
# Prerequisites: source Kafka (docker/services/kafka.yml), target Kafka (docker/services/kafka-target.yml),
#                MM2 Connect (docker/services/mirror-maker-2.yml) running; MM2 REST API on port 8086.
