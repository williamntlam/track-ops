...
# 1. Start PostgreSQL
start_service "postgres" "docker/services/postgres.yml"

# 2. Start Redis
start_service "redis" "docker/services/redis.yml"

# 3. Start Kafka (KRaft mode - no ZooKeeper required)
start_service "kafka" "docker/services/kafka.yml"
...
if curl -sf http://localhost:8086/connectors > /dev/null 2>&1; then
    ...
    [ -f "./scripts/infra/mm2/setup-mm2-connectors.sh" ] && ./scripts/infra/mm2/setup-mm2-connectors.sh || true
else
    print_warning "MirrorMaker 2 Connect not ready yet (connectors can be registered later: ./scripts/infra/mm2/setup-mm2-connectors.sh)"
fi

# 5. Start Prometheus
start_service "prometheus" "docker/services/prometheus.yml"

# 6. Start Grafana
start_service "grafana" "docker/services/grafana.yml"
...
# Start Order Service
start_service "trackops-server" "docker/services/trackops-server.yml"

# Start Inventory Service
print_status "Starting Inventory Service..."
docker compose -f docker/services/inventory-service.yml up -d
sleep 10
...
docker compose -f docker/services/event-relay-service.yml up -d
sleep 10
